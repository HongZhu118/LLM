...
def llama3_response(N,D,parent1,parent2):
    template = {
      "L": [],
      "K": [],}
    prompt = f"""
            I have two existing {N} by {D} dimensional numpy array P={parent1} and O={parent2}.\
    ...
    while True:
        try:
            ...
            if len(L_values) == len(K_values) == N*D:
                for i in range(N):
                    offspring.append(L_values[i * D:i * D + D])
                    offspring.append(K_values[i * D:i * D + D])
                break
        except :
            print("Please")
            continue
